...
⚙️ Operating Principle and Optimization FlowThe script uses fmincon (configured with the active-set algorithm) to iteratively search for the optimal trajectory.
1. Initialization and SetupData Loading: SPICE kernels (LSK, SPK, PCK) for Earth, Moon, Sun, and Apophis are loaded.
Mission Constants: Physical parameters are defined, including Asteroid Mass ($\text{M}_{\text{ast}}$), Specific Impulse ($\text{I}_{\text{sp}}$), and the Momentum Enhancement Factor ($\beta$).
...
Time Parameters: "α0​ (Launch time fraction), α1​ (DSM time fraction), TOF (Total Flight Time).","αi​∈[0,0.9], TOF∈[1000,2000] days"
Launch Parameters: "Hyperbolic Excess Velocity (Δv0​), In-plane/Out-of-plane angles (α,δ), initial S/C mass (msc0​).","Δv0​∈[1.5,5.5] km/s, msc0​∈[100,3000] kg"
...
Equality ($\mathbf{ceq}$) Trajectory Continuity - Ensures the propagated state from $\mathbf{x}_1$ meets $\mathbf{x}_2$, and $\mathbf{x}_2$ meets $\mathbf{x}_3$.
Equality ($\mathbf{ceq}$) Asteroid Match - Enforces that the S/C final position ($\mathbf{x}_{3, 1:3}$) exactly matches the Apophis position at $t_{\text{IMP}}$.
Inequality ($\mathbf{c}$) $\Delta V$ Budget - Restricts the total $\Delta V$ to be less than $5 \text{ km/s}$: $\Delta v_0 + \Delta v_{\text{DSM}} \le 5$.
Inequality ($\mathbf{c}$) Timing Check - Ensures $t_0$ and $t_{\text{IMP}}$ adhere to the mission windows.
...
![Earth - Asteroid Distance Before and After Impact at TCA](Results/Distance_Earth_AtTCA.fig)
...
![Mission Overview Result](Results/MissionOverviewECLIPJ2000.fig)
